package com.esp1617.albertomoretto.foodify;

/**
 * Created by leonardo on 04/05/17.
 */
//classe che contiente tutte le costanti usate nel progetto
public class FoodifyConstants {

    //Nomi di pietanze, ingredienti e dimensioni
    public static final String DEFAULT_ITEMS_READY = "";
    public static final String FRIES = "fries";
    public static final String BASIC_HOTDOG_INGREDIENTS="Basic Hotdog";
    public static final String BASIC_FRIES_INGREDIENTS="Basic Fries";
    public static final String MEAT_HABURGER_CLASSIC_NAME ="Classic Hamburger";
    public static final String MEAT_HABURGER_DOUBLE_NAME ="Double Hamburger";
    public static final String MEAT_HABURGER_JUICY_NAME ="Juicy Hamburger";
    public static final String BREAD_HAMBURGER_SESAME_BREAD_NAME = "Sesame Bread";
    public static final String BREAD_HAMBURGER_WHOLE_BREAD_NAME = "Whole Bread";
    public static final String BREAD_HAMBURGER_BUCKWHEAT_BREAD_NAME = "Buckwheat Bread";
    public static final String BREAD_CATEGORY = "Bread";
    public static final String MEAT_CATEGORY ="Meat";
    public static final String ADDITION_CATEGORY = "Addition";
    public static final String DRINK_SIZE_SMALL_NAME = "Small";
    public static final String DRINK_SIZE_MEDIUM_NAME = "Medium";
    public static final String DRINK_SIZE_LARGE_NAME = "Large";
    public static final String DRINK_TYPE_COKE_NAME = "Coke";
    public static final String DRINK_TYPE_DIET_COKE_NAME = "Diet coke";
    public static final String DRINK_TYPE_SPRITE_NAME = "Sprite";
    public static final String DRINK_TYPE_WATER_NAME = "Water";
    public static final String SIZE_CATEGORY = "Size";
    public static final String SALAD = "Salad";
    public static final String TOMATOES = "Tomatoes";
    public static final String ONIONS = "Onions";
    public static final String CUCUMBER = "Cucumber";
    public static final String BREADCRUMBS = "BreadCrumbs";
    public static final String MUSTARD = "Mustard";
    public static final String MAYONNAISE = "Mayonnaise";
    public static final String KETCHUP = "Ketchup";
    public static final String BARBECUE_SAUCE = "Barbecue Sauce";
    public static final String ICE_CREAM_SANDWICH = "Ice Cream Sandwich";
    public static final String JELLY_BEAN = "Jally Bean";
    public static final String KIT_KAT = "Kit-kat";
    public static final String LOLLIPOP = "Lollipop";
    public static final String MARSHMELLOW = "Marshmellow";
    public static final String NOUGAT = "Nougat";
    public static final String TYPE_CATEGORY = "Type";
    public static final String HOTDOG = "hotdog";

    //Prezzi di pietanze e ingredienti
    public static final float BREAD_HAMBURGER_SESAME_BREAD_PRICE = 2;
    public static final float BREAD_HAMBURGER_WHOLE_BREAD_PRICE = 2.5f;
    public static final float BREAD_HAMBURGER_BUCKWHEAT_BREAD_PRICE = 3;
    public static final float MEAT_HABURGER_CLASSIC_PRICE = 3;
    public static final float MEAT_HABURGER_DOUBLE_PRICE = 6;
    public static final float MEAT_HABURGER_JUICY_PRICE = 4.5f;
    public static final float BASIC_HOTDOG_INGREDIENTS_PRICE = 4;
    public static final float BASIC_FRIES_INGREDIENTS_PRICE = 3;
    public static final float DRINK_SIZE_SMALL_PRICE = 1;
    public static final float DRINK_SIZE_MEDIUM_PRICE = 2.5f;
    public static final float DRINK_SIZE_LARGE_PRICE = 4;
    public static final float DRINK_TYPE_COKE_PRICE = 1.5f;
    public static final float DRINK_TYPE_DIET_COKE_PRICE = 2;
    public static final float DRINK_TYPE_SPRITE_PRICE = 1.5f;
    public static final float DRINK_TYPE_WATER_PRICE = 0.5f;
    public static final float SALAD_PRICE = 0.5f;
    public static final float TOMATOES_PRICE = 0.5f;
    public static final float ONIONS_PRICE = 0.5f;
    public static final float CUCUMBER_PRICE = 0.5f;
    public static final float BREADCRUMBS_PRICE = 1;
    public static final float MUSTARD_PRICE = 1;
    public static final float MAYONNAISE_PRICE = 1;
    public static final float KETCHUP_PRICE = 1;
    public static final float BARBECUE_SAUCE_PRICE = 1;
    public static final float ICE_CREAM_SANDWICH_PRICE = 3;
    public static final float JELLY_BEAN_PRICE = 2.5f;
    public static final float KIT_KAT_PRICE = 2f;
    public static final float LOLLIPOP_PRICE = 1.5f;
    public static final float MARSHMELLOW_PRICE = 3f;
    public static final float NOUGAT_PRICE = 3.5f;

    //Valori di aiuto per layout
    public static final int PADDING_LEFT_RIGHT = 100;

    public static final int PADDING_TOP_DOWN = 180;
    //Dimensione testo dell'ordine, valore d'aiuto per layout
    public static final int TEXT_SIZE_ORDER_MESSAGE = 35;

    //Valore massimo e valore di default dell'ID di una notifica
    public static final int  MAX_NOTIFY_ID = 50000;
    public static final int DEFAULT_ORDER_ID = 1;

    //Valore standard di update del conto e dell'incremento
    public static final float STANDARD_ACCOUNT_UPDATE_VALUE = 1.5f;
    public static final float DEFAULT_ACCOUNT_VALUE = 0.0f;







}
